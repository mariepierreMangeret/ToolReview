...
function buildArrayIrregularVerbs () {
	var tableIrregularVerbsHtml = "";
	...
	return tableIrregularVerbsHtml;
}

function validateIrregulatVerbs() {
	var tableIrregularVerbsRows = document.getElementById("table_irregular_verbs").rows;

	for(var i=0; i<tableIrregularVerbsRows.length; i++) {
		var tableIrregularVerbsCells = tableIrregularVerbsRows[i].cells;

		for(var j=0; j<tableIrregularVerbsCells.length; j++) {
			if ( tableIrregularVerbsCells[j].children[0] != undefined && tableIrregularVerbsCells[j].children[0].value == verbs[i-1][j] ) {
				tableIrregularVerbsCells[j].children[0].style.color = "#04B73D";
				tableIrregularVerbsCells[j].style.borderColor = "#04B73D";
			} else if( tableIrregularVerbsCells[j].children[0] != undefined && tableIrregularVerbsCells[j].children[0].value != verbs[i-1][j] ) {
				tableIrregularVerbsCells[j].children[0].style.color = "#C20306";
				tableIrregularVerbsCells[j].style.borderColor = "#C20306";
			}
		}
	}
	window.scrollTo(0, 0);
...
document.getElementById('table_irregular_verbs').innerHTML += buildArrayIrregularVerbs();